...
public class SupernodeHelper extends SummaryGraphModule {
    ...
    public boolean edgeExist(int from, int to){
        int[] vIdx = {getNodeMap(from), getNodeMap(to)};
        IntArrayList neis=getNeighbors(vIdx[0]);
        for(int temp: neis){
            if(temp==vIdx[1])return true;
        }
        return false;
    }

    public int nodeDegree(int from){
        return getNeighbors(getNodeMap(from)).size();
    }
