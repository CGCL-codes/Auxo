...
public class MoSSoMCMC extends SupernodeHelper {
    ...
    public MoSSoMCMC(boolean directed, int _interval){
        super(directed);
        if(directed){
            try {
        ...
        if (add) {
            ...
            for(int _u: getMembers(Su)){
                for(int _v: getMembers(Sv)){
                    ...
                    if(e == 0){
                        // (_u, _v) is not in Cp
                        Cm.addEdge(_u, _v); Cm.addEdge(_v, _u);
                    }else{
                        // (_u, _v) is in Cp
                        Cp.deleteEdge(_u, _v); Cp.deleteEdge(_v, _u);
                    }
                }
            }
        } else {
            ...
            for(int _u: getMembers(Su)){
                for(int _v: getMembers(Sv)){
                    ...
                    if(e == 0){
                        // (_u, _v) is not in Cm
                        Cp.addEdge(_u, _v); Cp.addEdge(_v, _u);
                    }else{
                        // (_u, _v) is in Cm
                        Cm.deleteEdge(_u, _v); Cm.deleteEdge(_v, _u);
                    }
                }
            }
        }
    ...
    private boolean tryNodalUpdate(final int v, final double beta){
        ...
        // draw random neighbor t
        int t = Nv.getInt(randInt(0, len-1)), St = V.getInt(t);

        int S = -1;
        double sn = (double) getNumberOfSupernodes();
        double eps = 1.0 / n;
        if(randDouble() < (sn * eps) / (getTotalEdgeCount(St) + (sn * eps))){
            // draw randomly
            if(getSize(R) == 1) S = R;
            else S = newSupernode();
            if(randInt(0, getNumberOfSupernodes()) > 0){
                while(getSize(S) == 0){
                    S = randInt(0, n-1);
                }
            }
            ...
            int rv = randInt(1, getTotalEdgeCount(St));
            Int2IntMap.FastEntrySet Nt = getSupernodeNeighborsAndWeights(St);
            for(Int2IntMap.Entry _s: Nt){
                if (rv <= _s.getIntValue()) {
                    S = _s.getIntKey();
                    break;
                } else {
                    rv -= _s.getIntValue();
        ...
        // compute unnormalized proposals, q_r->s (Qs), and q_s->r (Qr)
        double Qr = 0, Qs = 0;
        ...
        for(int u: Nv) {
            int U = V.getInt(u);
            long totalEdgeCount = getTotalEdgeCount(U);
            long loopCount = getEdgeCount(U, U) / 2;
            totalEdgeCount -= loopCount;
            if(U == S) Qs += (loopCount + eps) / (double) (totalEdgeCount + (sn * eps));
            else Qs += (getEdgeCount(U, S) + eps) / (double) (totalEdgeCount + (sn * eps));

            long newTotalEdgeCount = totalEdgeCount;
            if(U == R){
                newTotalEdgeCount -= (len - rDelta);
                newEdgeCount = getEdgeCount(R, R) / 2 - rDelta;
            }else if(U == S){
                newTotalEdgeCount += (len - sDelta);
                newEdgeCount = getEdgeCount(S, R) + (rDelta - sDelta);
            }else{
                newTotalEdgeCount = totalEdgeCount;
                newEdgeCount = getEdgeCount(U, R) - edgeDelta.getOrDefault(U, 0);
            }
            Qr += (newEdgeCount + eps) / (double) (newTotalEdgeCount + (sn * eps));
        }


        // compute Delta of |P| + |Cp| + |Cm| and choose hyperparameter beta
        ...
        // compute acceptance ratio using delta, beta and proposal
        long delta = (after - before);
        double acceptanceRatio = min(Math.exp(-beta * delta) * (Qr / Qs), 1.0);
    ...
    public void processEdge(final int src, final int dst, final boolean add) {
        ...
        if(getDegree(src) > 0){
            for(int v: getNeighbors(src)){
                tryNodalUpdate(v, beta);
            }
        }else{
